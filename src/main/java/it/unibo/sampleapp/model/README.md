...
- `MultithreadGameModel` / `TaskBasedGameModel`: specializations that wire the collision and step resolution strategies.
- `physics/PhysicsEngine`: pure physics step logic (movement, collisions, holes).
- `physics/collision/*`: collision resolution abstraction and implementations (sequential, multithread, task-based).
- `physics/step/*`: per-ball step resolution abstraction and implementations (sequential, multithread).
...
- **Strategy wiring**: collision and step resolution strategies are injected into `PhysicsEngine` by the concrete game-model specialization.
